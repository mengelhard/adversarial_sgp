...
### Notes:
### 1) Simplify the generator(s)
### 2) Add ref and loss for kernel hyperparams
### 3) ???
### 4) ???
...
def generator(initial_model,type='simple',tau_initial=1e-4,learn_tau=True,nn_depth=3,reuse=False):
    """
    ...
    with tf.variable_scope("generator", reuse=reuse) as scope:
        ...
        if type == 'simple':
            ...
            net = tf.random_uniform((FLAGS.batch_size,
            ...
            for i in range(nn_depth):
                net = slim.fully_connected(net,initial_model['n_components'], 
                    activation_fn=tf.nn.elu,scope='fc_%d' % (i+1))
            weights = gumbel_softmax_sample(tf.nn.log_softmax(net,axis=1),
                temperature=tau,num_samples=FLAGS.batch_size)
...
def khp_generator(khp_samples,ndims,ard=FLAGS.ard,reuse=False,
    initial_sls=FLAGS.sls,initial_sfs=FLAGS.sfs,initial_noise=FLAGS.noise):

    with tf.variable_scope("generator", reuse=reuse) as scope:

        if ard:
            sls_means = slim.model_variable('sls_means',
                shape=ndims,
                initializer=tf.constant_initializer(np.log(initial_sls))
                )
            sls_sds = slim.model_variable('sls_sds',
                shape=ndims,
            ...
            sls_mean = slim.model_variable('sls_mean',
                shape=(),
                initializer=tf.constant_initializer(np.log(initial_sls))
                )
            sls_sd = slim.model_variable('sls_sd',
                shape=(),
                initializer=tf.constant_initializer(1.)
                )
        ...
        sfs_mean = slim.model_variable('sfs_mean',
            shape=(),
            initializer=tf.constant_initializer(np.log(initial_sfs))
            )
        sfs_sd = slim.model_variable('sfs_sd',
            shape=(),
            initializer=tf.constant_initializer(1.)
            )
        ...
        noise_mean = slim.model_variable('noise_mean',
            shape=(),
            initializer=tf.constant_initializer(np.log(initial_noise))
            )
        noise_sd = slim.model_variable('noise_sd',
            shape=(),
            initializer=tf.constant_initializer(1.)
            )
...
def kmeans_mixture_model(data,n_clusters=100,random_state=0):
    ...
    c_widths = np.array([np.sum(x_centered[lbls==c,:]**2,axis=0)/np.sum(lbls==c) 
...
def sgp_model(tx,ty,vx,vy):
    ...
    target_distribution = nlog_sgp_marglik(tx,ty,z,sls,sfs,noise,obj=FLAGS.sgp_approx)
