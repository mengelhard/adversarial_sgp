...
def generator_gumbel(eps_unif,eps_gauss,tau_initial=1.,reuse=False):
    """
...
def main():
    ...
    def run_training(sess, niter=NITER, ninitial=100, figdir=FIGDIR):
        ...
        for i in progress:
            ...
            if i%dispfreq == 0:
                ...
                gen_weights = np.sum(weights_ >= np.max(weights_,axis=1)[:,np.newaxis],axis=0)/BATCH_SIZE
